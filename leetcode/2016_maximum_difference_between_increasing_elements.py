...
class Solution:
    def maximumDifference(self, nums: List[int]) -> int:
        previous_min, result = float('inf'), -1
        for num in nums:
            if num - previous_min > 0:
                result = max(result, num - previous_min)
            previous_min = min(previous_min, num)
        return result
...
if __name__ == '__main__':
    ...
    assert 4 == solution.maximumDifference([7, 1, 5, 4])
    assert -1 == solution.maximumDifference([9, 4, 3, 2])
    assert 9 == solution.maximumDifference([1, 5, 2, 10])
    assert -1 == solution.maximumDifference([
