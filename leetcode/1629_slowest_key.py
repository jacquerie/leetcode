...
class Solution:
    def slowestKey(self, releaseTimes: List[int], keysPressed: str) -> str:
        current_best, current_time, result = releaseTimes[0], releaseTimes[0], keysPressed[0]
        for time, key in zip(releaseTimes[1:], keysPressed[1:]):
            if time - current_time > current_best or time - current_time == current_best and key > result:
